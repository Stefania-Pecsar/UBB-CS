     ...
     def fit(self, x: list, y: list):
        ...
        for batch_start in range(0, len(x), batch_size):
            ...
            for i in range(len(batch_x)):
                y_computed = self.eval(batch_x[i])
                err = y_computed - batch_y[i]
                errors.append(err)
            batch_error = mean(errors)
            

            for i in range(len(batch_x)):
                for j in range(len(batch_x[0])):
                    self.coef_[j] -= self.learning_rate * batch_error * batch_x[i][j]
                # Actualizare intercept (bias)
                self.coef_[-1] -= self.learning_rate * batch_error * 1
    
    # Separa intercept (w0) de coeficienții propriu-ziși
...
for i in range(0, len(trainInputs), 1):
    ...
    regressor.fit(X_batch, y_batch)  # Folosim metoda fit a clasei MyGDRegression
...
def manual_mean_squared_error(true_values, predicted_values):
    # Convertim listele în numpy array pentru calcul eficient
    y_true = np.array(true_values)
    y_pred = np.array(predicted_values)
    
    # Calculăm diferența la pătrat
...
error = manual_mean_squared_error(testOutputs, computedTestOutputs)
print("GD-tool")
