...
import org.example.AF.AutomateFinite;
import org.example.AF.FileManager;
import org.example.LA.FIPItem;
import org.example.LA.TSHashTable;
import org.example.LA.TSValue;
...
public class LexicalAnalyzer {
    ...
    private final TSHashTable ts = new TSHashTable(30);
    private final List<AnalyzerError> errorList = new ArrayList<>();
    private AutomateFinite finiteAutomataIntegers;
    private AutomateFinite finiteAutomataRealNumbers;
    private AutomateFinite finiteAutomataIdentifiers;
    ...
    private void writeTSToFile(TSHashTable table) {
        try (PrintWriter writer = new PrintWriter(new FileWriter("TS1.csv"))) {
    ...
    private void writeFIPToFile() {
        try (PrintWriter writer = new PrintWriter(new FileWriter("FIP1.csv"))) {
    ...
    private void readAutomatas() throws IOException {
        finiteAutomataIntegers = FileManager.readAF("src/main/resources/automata_integers.txt");
        finiteAutomataRealNumbers = FileManager.readAF("src/main/resources/automata_floats.txt");
        finiteAutomataIdentifiers = FileManager.readAF("src/main/resources/automata_ids.txt");
    }
