...
import org.example.TSHashTable;
...
public class LexicalAnalyzer {
    ...
    private boolean processLine(String line, int lineNumber) {
        ...
        Pattern tokenPattern = Pattern.compile(
                "\"([^\"]*)\"|" +               // strings
                        ...
                        "\\d+|" +                       // ints
                        "<<|>>|<=|>=|==|!=|" +          // operators
                        "[a-zA-Z_][a-zA-Z_0-9]*\\.[a-zA-Z_][a-zA-Z_0-9]*|" + // struct members like c.r
        ...
        while (matcher.find()) {
            ...
            if (token.matches("[a-zA-Z_][a-zA-Z_0-9]*\\.[a-zA-Z_][a-zA-Z_0-9]*")) {
                String[] parts = token.split("\\.");
                // Process first part (e.g., "c")
                processIdentifier(parts[0], lineNumber);
                // Process dot operator
                atoms.add(".");
                FIP.add(new FIPItem(".", -1, atomCodes.get("."), "."));
                // Process second part (e.g., "r")
                processIdentifier(parts[1], lineNumber);
                continue;
            }
    ...
    // Write TS to files
    private void writeTSToFile(String filename, TSHashTable table, String type1, String type2) {
    ...
    private void processIdentifier(String token, int lineNumber) {
        TSValue tsValue = new TSValue(token, -1, atomCodes.get("ID"), "ID");
        int pos = ts.insert(tsValue);
        tsValue.setPosition(pos);
        atoms.add("ID:" + token);
    ...
    public void lexicalAnalysis(String filename, String jsonFile) {
        try {
            ...
            try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
                ...
                // Output results
                writeTSToFile("LAB.csv", ts, "ID", "CONST");
